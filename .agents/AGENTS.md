...
- **Liquid over JS (Minimal JS):** Whenever adding a new feature, it MUST be fully customizable via the Theme Editor. Additionally, always prefer native Liquid rendering and CSS over JavaScript. Use minimal JS only when absolutely necessary for interactivity.
- **Default Demonstration:** Whenever building a new feature, section, or block, ALWAYS add a demonstration of it to the theme's default templates (e.g., `index.json`, `product.json`) or ensure it has clear `presets` in the schema so it is immediately visible to the user without manual setup.
- **Surface Snippet Settings:** When rendering a snippet inside a section, ALWAYS pull its customization options up into the parent section's or block's schema so merchants can configure the snippet directly from the Theme Editor.
